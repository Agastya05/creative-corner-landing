...
import { artistApi, type Artist, type Artwork } from "@/lib/api";
...
export default function ArtistPage({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    ...
    setError(null);
    Promise.all([
      artistApi.getArtistBySlug(slug),
      artistApi.getArtworksByArtist(slug),
    ])
      .then(([a, w]) => {
        if (cancelled) return;
        setArtist(a);
  ...
  const jsonLd = {
    ...
    address: { "@type": "PostalAddress", addressLocality: artist.city.split(",")[0], addressCountry: "IN" },
  };

  return (
    <main className="min-h-screen" style={{ background: artist.theme.bg, color: artist.theme.text }}>
    ...
    <header className="relative overflow-hidden">
      ...
      <div className="relative grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] items-center gap-10 px-8 py-20 md:py-28">
        ...
        <div className="flex flex-col items-center text-center">
          ...
          <p className="mt-4 text-[0.7rem] tracking-[0.4em] uppercase text-cream/80">About the Artist</p>
          ...
          {items.map((a) => (
            <li key={a.id} className="group flex flex-col">
              <div
                ...
                style={{ background: `linear-gradient(135deg, ${a.palette[0]} 0%, ${a.palette[1]} 100%)` }}
                role="img"
