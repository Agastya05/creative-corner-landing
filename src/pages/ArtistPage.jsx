...
/**
 * Plain React page — no framework lock-in.
 * Fetches via the swappable `artistApi` adapter (mock or REST).
 */
export default function ArtistPage({ slug }) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError(null);
    Promise.all([artistApi.getArtistBySlug(slug), artistApi.getArtworksByArtist(slug)])
      .then(([a, w]) => {
        if (cancelled) return;
        setArtist(a);
        setArtworks(w);
  ...
  if (loading) {
    return (
      <main className="grid min-h-screen place-items-center bg-[var(--ochre)] text-[var(--ink)]">
        <p className="text-[0.7rem] tracking-[0.4em] uppercase opacity-70">Loading…</p>
      </main>
    );
  }
    ...
    return (
      <main className="grid min-h-screen place-items-center bg-[var(--ochre)] text-[var(--ink)]">
        <div className="text-center">
          <p className="font-display text-3xl">Artist not found</p>
          <p className="mt-3 text-sm opacity-70">{error ?? "Unknown error"}</p>
        </div>
      </main>
    );
  }
  ...
  const jsonLd = {
    ...
    "@type": "Person",
    name: artist.name,
    jobTitle: artist.job_title,
    nationality: artist.nationality,
    birthPlace: { "@type": "Place", name: artist.city },
    worksFor: { "@type": "Organization", name: "Independent Studio Practice" },
    knowsAbout: [artist.movement, "Contemporary Art", "Color Theory", "Mixed Media Painting"],
    alumniOf: artist.education.map((e) => ({ "@type": "CollegeOrUniversity", name: e.school })),
    award: artist.accolades.map((ac) => `${ac.name} (${ac.year})`),
    address: {
      "@type": "PostalAddress",
      addressLocality: artist.city.split(",")[0],
      addressCountry: "IN",
    },
  };
  ...
  return (
    <main
      ...
      style={{ background: artist.theme.bg, color: artist.theme.text }}
    >
      <script
      ...
      <ArtworkForSale artist={artist} artworks={artworks} />
    </main>
  );
}
...
function Hero({ artist }) {
  ...
  return (
    <header className="relative overflow-hidden">
      ...
      <div
        aria-hidden
        className="absolute inset-x-0 bottom-0 h-1/2 bg-gradient-to-b from-transparent to-[var(--ochre)]"
      />
        ...
        <div className="flex animate-[marquee_30s_linear_infinite] gap-12 whitespace-nowrap">
          {Array.from({ length: 4 }).map((_, i) => (
            <span key={i} className="flex items-center gap-12 text-white/90">
              <span>{artist.name}</span> · <span>{artist.movement}</span> ·{" "}
      ...
      <div className="relative grid grid-cols-1 md:grid-cols-[1fr_auto_1fr] items-center gap-10 px-8 py-20 md:py-28">
        ...
        <div className="flex flex-col items-center text-center">
          ...
          <h1 className="mt-6 font-display text-5xl md:text-6xl text-cream">{artist.name}</h1>
          <p className="mt-2 text-xs tracking-[0.4em] uppercase text-cream/80">{artist.city}</p>
          <div className="mt-6 flex flex-wrap justify-center gap-3 text-[0.7rem] tracking-[0.3em] uppercase text-cream">
            ...
            <span className="border border-cream/70 px-5 py-2.5">
              {artist.artwork_for_sale_subtitle}
            </span>
          </div>
          ...
          <p className="mt-6 max-w-2xl font-italic italic text-xl md:text-2xl leading-relaxed text-cream">
            {artist.name} creates stunning abstract compositions that blend traditional and
            contemporary techniques. Her work explores the interplay between color, form, and
            emotion.
          </p>
        </div>
    ...
    >
      ...
      <div className="grid gap-16 md:grid-cols-[1.4fr_1fr]">
        <article>
          ...
          <div
            className="mt-8 space-y-5 text-[0.95rem] leading-[1.85] opacity-85"
            ...
            <ul className="mt-6 space-y-5">
              {artist.education.map((e) => (
                ...
                >
                  ...
                  <p className="text-xs opacity-70" itemProp="recognizedBy">
                    {e.school}
                  </p>
                  <p className="text-[0.65rem] tracking-[0.3em] uppercase opacity-50 mt-1">
                    {e.year}
                  </p>
                </li>
              ))}
            ...
            <ul className="mt-6 space-y-4">
              {artist.accolades.map((a) => (
                <li key={a.name} className="flex gap-3" itemProp="award">
                  <span aria-hidden className="mt-1" style={{ color: "var(--ochre-deep)" }}>
                    ★
                  </span>
                  <div>
      ...
      <div className="mx-auto max-w-7xl px-6 py-24 md:py-32">
        <div className="grid gap-10 md:grid-cols-[1fr_auto_1fr] md:items-end">
          ...
          <p className="text-sm leading-relaxed text-cream/70 md:text-right">
            A look at the daily rituals, palettes and unfinished canvases that shape{" "}
            {artist.name.split(" ")[0]}&rsquo;s practice.
          </p>
        </div>
...
/* ---------- EXHIBITIONS ---------- */
function Exhibitions({ artist }) {
  return (
    <section
      aria-labelledby="exhibitions-heading"
      className="mx-auto max-w-6xl px-6 py-24 md:py-32"
    >
      <h2 id="exhibitions-heading" className="font-display text-5xl">
        Exhibitions
      </h2>
...
function ExhibitionList({ title, items }) {
  return (
    <div>
      ...
      <ul className="mt-8 space-y-8">
        {items.map((e) => (
          ...
          >
            <p className="font-bold" itemProp="name">
              {e.name}
            </p>
            <p className="text-sm opacity-70" itemProp="location">
              {e.venue}
            </p>
...
/* ---------- ARTWORK FOR SALE ---------- */
function ArtworkForSale({ artist, artworks }) {
  const [expanded, setExpanded] = useState(false);
  const items = expanded ? artworks : artworks.slice(0, 5);
  const titleParts = artist.artwork_for_sale_title.split(" ");
    ...
    <section aria-labelledby="shop-heading" className="bg-[var(--cream)] text-[var(--ink)]">
      <div className="mx-auto max-w-7xl px-6 py-24 md:py-32">
        <div className="text-center">
          ...
          >
            {titleLead} <span className="italic text-[var(--ochre-deep)]">{lastWord}</span>
          </h2>
          <p className="mt-3 text-[0.7rem] tracking-[0.4em] uppercase opacity-60">
            {artist.artwork_for_sale_subtitle}
          </p>
        </div>
