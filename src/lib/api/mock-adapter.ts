import type { Artist, ArtistAPI, Artwork } from "./types";
...
const PRIYA: Artist = {
  ...
  theme: { bg: "var(--ochre)", text: "var(--ink)", cream: "var(--cream)", accent: "var(--ochre-deep)" },
  images: {
  ...
  studio_quote: "The studio is the only place where colour stops being decoration and starts being a language.",
...
const ARTWORKS: Artwork[] = Array.from({ length: 50 }, (_, i) => ({
  id: `mock-artwork-${i + 1}`,
...
const ARTISTS: Record<string, Artist> = { [PRIYA.slug]: PRIYA };
const ARTWORKS_BY_SLUG: Record<string, Artwork[]> = { [PRIYA.slug]: ARTWORKS };

export const mockAdapter: ArtistAPI = {
